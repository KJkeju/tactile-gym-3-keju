...
from tg3.learning.supervised.image_to_feature.cnn.utils_plots import LearningPlotter, RegressionPlotter
from tg3.learning.supervised.image_to_feature.setup_train import setup_parse
...
if __name__ == '__main__':
    ...
    for args.dataset, args.predict, args.model in it.product(args.datasets, args.predicts, args.models):
        ...
        # set save dir
        save_dir = os.path.join(args.path, output_dir, args.dataset, 'predict_' + args.predict, args.model)

        # create task params
        label_params = load_json_obj(os.path.join(save_dir, 'model_label_params'))

        # load and plot predictions
        with open(os.path.join(save_dir, 'val_pred_targ_err.pkl'), 'rb') as f:
            pred_df, targ_df, err_df = pickle.load(f)

        error_plotter = RegressionPlotter(label_params, save_dir, 'error_plot_best.png')
