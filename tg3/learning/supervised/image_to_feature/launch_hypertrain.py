...
from tg3.learning.supervised.image_to_feature.setup_train import setup_training, setup_parse, csv_row_to_label
from tg3.utils.utils import save_json_obj, make_dir, seed_everything
...
def launch(args, space, max_evals=20, n_startup_jobs=10):
    ...
    for args.experiment, args.predict in zip(args.experiments, args.predicts):
        for args.model in args.models:
            ...
            train_data_dirs = [
                os.path.join(args.path, output_dir, args.experiment, d) for d in args.train_dirs
            ]
            val_data_dirs = [
                os.path.join(args.path, output_dir, args.experiment, d) for d in args.val_dirs
            ]

            # setup save dir
            save_dir = os.path.join(args.path, output_dir, args.experiment, 'predict_' + args.predict, args.model)
            make_dir(save_dir)
...
if __name__ == "__main__":
    ...
    args = setup_parse(
        ...
        robot='sim',
        sensor='tactip',
        experiments=['surface_zRxRy'],
        predicts=['pose_zRxRy'],
        models=['simple_cnn_hyp'], # first part of name determines model type 
    ...
    launch(args, space, max_evals=20, n_startup_jobs=8)
