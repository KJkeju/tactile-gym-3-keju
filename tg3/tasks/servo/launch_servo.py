...
def servo(
    ...
    for i in range(task_params['num_iterations']):
        ...
        servo = controller.update(pred_pose[:6]) # because pose and shear
...
def launch(args):
    ...
    for args.experiment, args.predict, args.model in it.product(args.experiments, args.predicts, args.models):
        for args.object, args.sample_num in zip(args.objects, args.sample_nums):
            ...
            # setup save dir
            save_dir = os.path.join(args.path, output_dir, args.experiment, 'servo_'+save_dir_name)
            ...
            # setup any plotters
            plotter = Contour3DPlotter(save_dir, name='contour.png', save_num=args.sample_num)
